      SUBROUTINE W3FI58(IFIELD,NPTS,NWORK,NPFLD,NBITS,LEN,KMIN,IERR)
...
C   07-08-30  N. WANG     ADDED ERROR MESSAGE FOR DYNAMIC RANGE OVERFLOW.
C
C USAGE:  CALL W3FI58(IFIELD,NPTS,NWORK,NPFLD,NBITS,LEN,KMIN,IERR)
C
C   INPUT:
C
...
C
C   EXIT STATES:  
C     IERR = 0, NO ERROR, IERR = 1, DYNAMIC RANGE OVERFLOW. N. WANG, 07-08-30
C
...
C
      BIGDIF = KMAX - KMIN
      IF (BIGDIF < 0.0) THEN 
        PRINT*,'W3FI58: DYNAMIC RANGE OVERFLOW', KMAX, KMIN
        IERR = 1
        RETURN
      ENDIF 
