...
! Coupling insertion->
     ...
     & sashal,newsas,mom4ice,mstrat,trans_trac,cal_pre,HFLX,EVAP,
...
!
     ...
     &,                    xcosz(im), suntim(im)    ! yth mar/08 cosz for compute suntim
      real(kind=kind_phys) sdiaga(IM,LEVS),      sdiagb(IM,LEVS)
...
!
      ...
      if(skip_cu_physics)then
         ktop(1) = cutop
         kbot(1) = cubot
         rain1(1)= dtp*bengsh(1)
      endif
!     if(bengsh(1).gt.0.)write(6,*)'cubot,cutop,raincv = ',
!    1                  cubot(1),cutop(1),bengsh(1)
!     if(bengsh(1).gt.0.)write(6,*)'kbot,ktop,rain1,dtf,dtp= ',
!    1                  kbot(1),ktop(1),rain1(1),dtf,dtp
...
!      print *,'SFCDSW,DSWSFC,radsl',SFCDSW,DSWSFC,radsl
...
!jbao        CALL OZPHYS(IX, IM, LEVS, KO3, DTP, GQ0(1,1,ntoz), GQ0(1,1,ntoz)
!jbao     &,             GT0, poz, prsl, prdout, pl_coeff, DEL, LDIAG3D
!jbao     &,             LGGFS3D, dq3dt(1,1,6), me)
...
!jbao        work1(i) = (log(coslat(i) / (nlons(i)*latg)) - dxmin) * dxinv
...
!    &              VVEL,xkt2,ncld)
     ...
     &                CLW,GQ0,GT0,gu0,gv0,rcs,CLD1D,
     &                RAIN1,KBOT,KTOP,kcnv,SLMSK,
     &                VVEL,ncld,ud_mf,dd_mf,dt_mf,sdiaga,sdiagb)
!hchuang code change 03/03/08 [r1L] add SAS modification of mass flux
!    &                VVEL,xkt2,ncld)
        endif
