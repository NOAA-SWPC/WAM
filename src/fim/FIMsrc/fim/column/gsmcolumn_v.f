...
!
      ...
      real (kind=kind_phys) r_val,r_fpvs,r_pp
!
!#######################################################################
!########################## Begin Execution ############################
!#######################################################################
...
!
          ...
          r_fpvs=FPVSL(r_val)
          r_pp=PP
          ESW=min(r_pp,r_fpvs)             ! Saturation vapor pressure w/r/t water
!         QSW=EPS*ESW/(PP-ESW)             ! Saturation mixing ratio w/r/t water
          ...
          IF (TC .LT. 0.) THEN
            ...
            r_fpvs=FPVSI(r_val)
            r_pp=PP
            ESI=min(r_pp,r_fpvs)           ! Saturation vapor pressure w/r/t ice
...
!
          ...
          THICK=THICK_col(L)         ! Layer thickness = RHO*DZ = -DP/G = (Psfc-Ptop)*D_ETA/(G*ETA_s
!
...
!
          ...
          PIACW=0.                   ! Cloud water collection (riming) by precipitation ice (kg/kg;
          PIACWI=0.                  ! Growth of precip ice by riming (kg/kg; >0)
          PIACWR=0.                  ! Shedding of accreted cloud water to form rain (kg/kg; >0)
          PIACR=0.                   ! Freezing of rain onto large ice at supercooled temps (kg/kg;
...
!
   ...
   !
            ...
            r_fpvs=FPVSI(r_val)
            r_pp=PP
            DUM=min(r_pp,r_fpvs)               ! Updated (dummy) saturation vapor pressure w/r/t ice
            DUM=RHgrd*EPS*DUM/(pp+epsm1*dum)   ! Updated (dummy) saturation specific humidity w/r/t
...
!
      ...
      r_fpvs=FPVSL(r_val)
      r_pp=pp
      ESW=min(r_pp,r_fpvs)                      ! Saturation vapor press w/r/t water
...
!       COND=DWV/(1.+XLV2*WS/(Tdum*Tdum))       ! Asai (1965, J. Japan)
        ...
        r_fpvs=FPVSL(r_val)
        r_pp=pp
        ESW=min(r_pp,r_fpvs)                    ! Updated saturation vapor press w/r/t water
...
!
      ...
      r_fpvs=FPVSI(r_val)
      r_pp=pp
      ESI=min(r_pp,r_fpvs)                      ! Saturation vapor press w/r/t ice
!     WS=RHgrd*EPS*ESI/(PP-ESI)                 ! Saturation mixing ratio
   ...
   !
        ...
        r_fpvs=FPVSI(r_val)
        r_pp=pp
        ESI=min(r_pp,r_fpvs)                    ! Updated saturation vapor press w/r/t ice
