...
!  hncrit set to 8000m and sigfac added to enhance elvmax mtn hgt
      parameter (sigfac=0.1)     ! MB3a expt test for ELVMAX factor (*j*)
...
!JR Define statement functions for sind, cosd, and atan2d in situations where 
!JR compiler doesn't support them (e.g. gfortran)
      real(kind=kind_phys) :: x, y, sind, cosd, atan2d
      sind(x) = sin (x*pi/180._kind_phys)
      cosd(x) = cos (x*pi/180._kind_phys)
      atan2d(x,y) = atan2 (x*pi/180._kind_phys, y*pi/180._kind_phys)
#endif
!
      DO I = 1, IM
         DUSFC(I) = 0.
...
! ----  for lm and gwd calculation points
      npt     = 0
      DO I = 1,IM
        IF ( (elvmax(i) .GT. HMINMT) .and. (hprime(i) .GT. 0.0001) )  then
           npt      = npt + 1
           ipt(npt) = i
        ENDIF
      ENDDO
...
!          endif
!        enddo
!     print *, ' mb: kdt,max(iwklm),jhit,phil,me=',kdt,ihit,jhit,phil(jhit,ihit),me
...
! ---                               Brunt-Vaisala Frequency
        BNV2LM(I,K) = (G+G) * RDZ * ( VTK(I,K+1)-VTK(I,K) ) / ( VTK(I,K+1)+VTK(I,K) )
...
!
      ...
      J = ipt(i)
       ...
       UDS(I,K) = RCS(I) * MAX(SQRT(U1(J,K)*U1(J,K) + V1(J,K)*V1(J,K)), minwnd)
! --- Test to see if we found Zb previously
            IF (IDXZB(I) .eq. 0 ) then
       PE(I) = PE(I) + BNV2lm(I,K)*(G*ELVMAX(J) - phil(J,K))*(PHII(J,K+1) - PHII(J,K))/(G*G)
! --- KE
...
!
!     if(lprnt) print *,' BNV2bar,BNV2lm=',bnv2bar(ipr),BNV2lm(ipr,:)
!     if(lprnt) print *,' ipr,IDXZB,UDS=',ipr,IDXZB(ipr),UDS(ipr,:)
!     if(lprnt) print *,' PE,UP,EK=',PE(ipr),UP(ipr),EK(ipr)
         DO I = 1, npt
         J    = ipt(i)
! --- Calc if N constant in layers (Zb guess) - a diagnostic only.
       ZBK(I) =  ELVMAX(J) - SQRT(UBAR(I)**2 + VBAR(I)**2)/BNV2bar(I)
         ENDDO
!     if(lprnt) print *,' iwklm,ZBK=',iwklm(ipr),ZBK(ipr),IDXZB(ipr)
!     if(lprnt) print *,' Zb=',PHIL(ipt(ipr),IDXZB(ipr))/G
...
!      print *,' in gwdps_lm.f 9  =',i,j,IDXZB(i),me
      ...
      DO K = IDXZB(I), 1, -1
        IF ( PHIL(J,IDXZB(I)) .gt.  PHIL(J,K) ) then
          ZLEN = SQRT( ( PHIL(J,IDXZB(I)) - PHIL(J,K) ) / ( PHIL(J,K ) + G * hprime(J) ) )
! --- lm eq 14:
...
!            if(lprnt) then 
!      print *,' in gwdps_lmi.f 10 npt=',npt,i,j,idxzb(i),DBTMP,R,me
!      print *,' in gwdps_lmi.f 11   K=',k,ZLEN,cosD(ANG(I,K)),me
...
!  WARNING  KINT = KREF !!!!!!!!!
      ...
      enddo
!
!     if(lprnt) print *,' ubar=',ubar,' vbar=',vbar,' ulow=',ulow,' veleps=',veleps
!
      DO I = 1,npt
        J      = ipt(i)
        BNV    = SQRT( BNV2bar(I) )
...
!             TEM1 = XLINV(I)*(RO(I,KP1)+RO(I,K))*BRVF*VELCO(I,K)*0.5
              TEM1 = XLINV(I)*(RO(I,KP1)+RO(I,K))*BRVF*0.5 * max(VELCO(I,K),0.01)
...
!       SCOR(I) = 1.0 / (RCS(I) * PSTAR(I))
      ...
      DO K = 1,KM
        DO I = 1,npt
          TAUD(I,K) = G * (TAUP(I,K+1) - TAUP(I,K)) * SCOR(I) / DEL(ipt(I),K)
        ENDDO
      ENDDO
!
...
!
      ...
      ENDDO
!
!     if(lprnt) print *,' before  A=',A(ipt(ipr),:)
!     if(lprnt) print *,' before  B=',B(ipt(ipr),:)
...
!          if ( ABS(DBIM * U1(J,K)) .gt. .01 ) print *,' in gwdps_lmi.f KDT=',KDT,I,K,DB(I,K), &
...
!
        ...
        ENDDO
      ENDDO
!     if(lprnt) print *,' in gwdps_lm.f after  A=',A(ipt(ipr),:)
!     if(lprnt) print *,' in gwdps_lm.f after  B=',B(ipt(ipr),:)
!     if(lprnt) print *,' DB=',DB(ipr,:)
      DO I = 1,npt
        J          = ipt(i)
