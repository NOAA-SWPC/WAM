      subroutine sascnvn(im,ix,km,jcap,delt,del,prsl,ps,
     &     phil,ql,
     &     q1,t1,u1,v1,rcs,cldwrk,rn,kbot,ktop,kcnv,slimsk,
     &     dot,ncloud,ud_mf,dd_mf,dt_mf,sdiaga,sdiagb)
...
!     
      do k = 1, km
        do i=1,im
! jbao / s.benjamin - change 9/6/2010
!jbao           if (ps(i).ge.600.) then
          if (prsl(i,k)*tx1(i) .gt. 0.04) kmax(i)  = k + 1
!jbao           else 
!jbao           if (prsl(i,k)*tx1(i) .gt. 0.20) kmax(i)  = k + 1
!jbao           end if
...
!             sdiaga(i,k)=dellat * xmb(i)
              q1(i,k) = q1(i,k) + dellaq(i,k) * xmb(i) * dt2
!             sdiagb(i,k)=dellaq(i,k) * xmb(i)
              tem = 1./rcs(i)
