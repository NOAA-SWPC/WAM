...
#   define HAVE_BLAS.
#
#   This macro has been updated to also check for the Intel Compiler -mkl
#   flag and to work when AC_LANG is Fortran/Fortran 77.
#
# LICENSE
#
...
AS_CASE("_AC_LANG",
	[Fortran|"Fortran 77"], [sgemm=sgemm; dgemm=dgemm],
	[AC_FC_FUNC(sgemm)
	 AC_FC_FUNC(dgemm)]
...
if test $ax_blas_ok = no; then
	...
	# MKL for other compilers (Intel, PGI, ...?)
                ...
                # Check if Intel compiler flag works first
                save_LIBS=$LIBS; LIBS="-mkl=sequential"
                AC_MSG_CHECKING([for $sgemm in $LIBS])
                AC_LINK_IFELSE([AC_LANG_CALL([], [$sgemm])],
                	[ax_blas_ok=yes;BLAS_LIBS="-mkl=sequential"], [])
                AC_MSG_RESULT($ax_blas_ok)
                LIBS=$save_LIBS

		# If not, check for specific architectures
		if test $ax_blas_ok = no; then
			# 64-bit
			if test $host_cpu = x86_64; then
